...
app.get("/sobre", (req, res) => {
  res
    .status(200)
    .send("Este é um projeto de API para gerenciar uma coleção de games.");
});
...
app.post("/api/games", (req, res) => {
  ...
  if (!titulo || !genero || !classificacao || anoLancamento === undefined) {
    return res
      ...
      .json({
        message:
          "Os campos titulo, genero, classificacao e anoLancamento são obrigatórios.",
      });
  }
  ...
  res
    .status(201)
    .json({ message: "Game adicionado com sucesso", game: novoGame });
});
...
app.put("/api/games/:id", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Game atualizado com sucesso", game: games[index] });
});
