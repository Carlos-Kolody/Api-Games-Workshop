...
app.post('/api/users/:userId/catalog', (req, res) => {
    ...
    const jaPossui = userCatalogs.some(item => item.usuarioId === Number(userId) && item.gameId === Number(gameId));
    if (jaPossui) {
        return res.status(409).json({ message: "Este game já está no catálogo do usuário." });
    }

    const novaEntrada = { id: nextCatalogId++, usuarioId: Number(userId), gameId: Number(gameId) };
...
app.delete('/api/users/:userId/catalog/:gameId', (req, res) => {
    ...
    const index = userCatalogs.findIndex(item => item.usuarioId === Number(userId) && item.gameId === Number(gameId));
